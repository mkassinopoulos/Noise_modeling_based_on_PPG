function [obj_function,CRF_sc,RRF_sc,BRF_sc, HR_conv,RF_conv,r_PRF_sc,yPred, HR_conv_MR, RF_conv_MR,yPred_PPG]=func_PRF_w_ePPG(P,Ts_10,HR,RF,ePPG, ind_BOLD_10,GS,only_OF,ind_train, ind_valid, filt_b, filt_a)
...
PPG_Fconv=conv(ePPG,IR_ppgF); PPG_Fconv_MR=PPG_Fconv(ind_BOLD_10+10*10);
PPG_Sconv=conv(ePPG,IR_ppgS); PPG_Sconv_MR=PPG_Sconv(ind_BOLD_10+10*10);
...
%     GS_clean = GS - regr(:,1:4)*B(1:4);        
    yPred_PPG = regr(:,5:7)*B(5:7);  r_PRF_sc(4) = corr(yPred_PPG(ind_valid),GS(ind_valid));
